...
use ratchet_lib::execution::retry::{
    BackoffStrategy, CircuitBreaker, CircuitState, ExecutionError, RetryError, RetryExecutor,
...
async fn test_circuit_breaker_state_transitions() {
    ...
    assert_eq!(
        ...
        "Should remain half-open until success threshold"
    );
...
async fn test_circuit_breaker_prevents_execution() {
    ...
    let result: Result<(), RetryError<TestError>> = executor
        .execute_with_circuit_breaker(|| async { Ok(()) }, &mut breaker)
        .await;
...
async fn test_circuit_breaker_with_execution_error() {
    ...
    let network_error: Result<(), RetryError<ExecutionError>> = executor
        .execute_with_circuit_breaker(
            || async { Err(ExecutionError::Network("Connection refused".to_string())) },
            &mut breaker,
        )
        .await;
    ...
    for i in 0..3 {
        ...
        if i < 2 {
            assert_eq!(
                breaker.state(),
                CircuitState::HalfOpen,
                "Should stay half-open during recovery"
            );
        }
    }
...
async fn test_multiple_circuit_breakers() {
    ...
    for _ in 0..3 {
        let _: Result<(), RetryError<ExecutionError>> = executor
            .execute_with_circuit_breaker(
                || async {
                    Err(ExecutionError::DatabaseConnection(
    ...
    let api_result: Result<&str, RetryError<TestError>> = executor
        .execute_with_circuit_breaker(|| async { Ok("API response") }, &mut api_breaker)
        .await;
