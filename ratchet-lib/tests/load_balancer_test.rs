...
use ratchet_lib::execution::load_balancer::{
    LeastLoadedStrategy, LoadBalancer, RoundRobinStrategy, WeightedRoundRobinStrategy,
    WorkerHealth, WorkerInfo,
};
...
async fn test_round_robin_load_distribution() {
    ...
    for (worker_id, count) in &selection_counts {
        assert_eq!(
            *count, 10,
            "Worker {} should have been selected 10 times",
            worker_id
        );
    }
}
...
async fn test_least_loaded_strategy_with_varying_loads() {
    ...
    for i in 0..3 {
        ...
        worker
            .metrics
            .tasks_in_flight
            .store(i as u32 * 2, Ordering::Relaxed);
        worker
            .metrics
            .memory_usage_mb
            .store((i + 1) as u64 * 1000, Ordering::Relaxed);
        worker
            .metrics
            .update_system_metrics((i + 1) as u64 * 1000, (i as f32 + 1.0) * 10.0);
    ...
    for _ in 0..5 {
        ...
        assert_eq!(
            selected, "worker-0",
            "Should select the least loaded worker"
        );
    }
}
...
async fn test_worker_health_monitoring() {
    ...
    assert!(
        !selected_workers.contains("unhealthy"),
        "Unhealthy worker should not be selected"
    );

    // Update unhealthy worker to healthy
    balancer
        .update_worker_health("unhealthy", WorkerHealth::Healthy)
        .await;
...
async fn test_worker_metrics_tracking() {
    ...
    assert!(
        (failure_rate - 0.4).abs() < 0.01,
        "Failure rate should be approximately 0.4"
    );
}
...
async fn test_load_balancer_statistics() {
    ...
    for i in 0..4 {
        ...
        worker
            .metrics
            .total_tasks
            .store((i + 1) as u64 * 10, Ordering::Relaxed);
        worker
            .metrics
            .total_failures
            .store(i as u64, Ordering::Relaxed);
...
async fn test_worker_capacity_limits() {
    ...
    assert!(
        selected.is_none(),
        "Worker at capacity should not be selected"
    );
