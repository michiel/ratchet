...
async fn test_pagination_parameters() {
    ...
    if response.status_code() != StatusCode::OK {
        ...
        panic!("Basic jobs endpoint failed with status {}: {}", response.status_code(), error_body);
    }
    ...
    if response.status_code() != StatusCode::OK {
        let error_body = response.text();
        panic!("Pagination failed with status {}: {}", response.status_code(), error_body);
    }
    ...
    if response.status_code() != StatusCode::OK {
        let error_body = response.text();
        panic!("Sorting failed with status {}: {}", response.status_code(), error_body);
    }
}
...
async fn test_job_creation_and_retrieval() {
    ...
    if response.status_code() != StatusCode::CREATED {
        let error_body = response.text();
        panic!("Job creation failed with status {}: {}", response.status_code(), error_body);
    }
...
async fn test_execution_filtering() {
    ...
    // Create test data
    use ratchet_lib::database::entities::{tasks, executions, ExecutionStatus};
    use sea_orm::{ActiveModelTrait, Set};
    ...
    for i in 0..3 {
        let exec_model = executions::ActiveModel {
            uuid: Set(Uuid::new_v4()),
            task_id: Set(task.id),
            status: Set(if i == 0 { ExecutionStatus::Completed } else { ExecutionStatus::Failed }),
            input: Set(json!({"index": i})),
            output: Set(Some(json!({"result": i}))),
            ..Default::default()
        };
        exec_model.insert(repos.database().get_connection()).await.unwrap();
    }
    ...
    let response = server.get(&format!("/executions?task_id={}", task.id)).await;
    assert_eq!(response.status_code(), StatusCode::OK);
    let body: Value = response.json();
    let executions = body["data"].as_array().unwrap();
...
async fn test_sequential_job_creation() {
    ...
    for i in 0..5 {
        ...
        if response.status_code() == StatusCode::CREATED {
            ...
            let error_body = response.text();
            println!("Job {} creation failed with status {}: {}", i, response.status_code(), error_body);
        }
    }
