use ratchet_lib::logging::{
    LogEvent, LogLevel, ErrorInfo, 
    ErrorPatternMatcher, MatchingRule, ErrorPattern, ErrorCategory,
    LLMExporter, LLMExportConfig, format_markdown_report,
};
use ratchet_lib::errors::{RatchetError, ErrorSeverity};
    ...
    let custom_pattern = ErrorPattern {
        ...
        matching_rules: vec![
            MatchingRule::All { rules: vec![
                MatchingRule::ErrorType { value: "AuthError".to_string() },
                MatchingRule::Any { rules: vec![
                    MatchingRule::MessagePattern { pattern: r"(?i)invalid.*token".to_string() },
                    MatchingRule::MessagePattern { pattern: r"(?i)expired.*credentials".to_string() },
                ]},
            ]},
        ],
        suggestions: vec![
        ...
        preventive_measures: vec![
            "Implement token refresh logic".to_string(),
        ],
...
fn test_llm_export_with_patterns() {
    ...
    assert!(report.suggested_prompts.iter().any(|p| p.contains("database connection timeout")));
}
...
fn test_data_summarization() {
    ...
    for i in 0..25 { // Will be truncated at >20
    ...
    let event = LogEvent::new(LogLevel::Error, "Data processing error")
        .with_error(error)
        .with_field("input_data", serde_json::json!({
            "string": large_string,
            "array": large_array,
            "object": large_object,
        }));
    ...
    assert!(report.execution_context.input_data_summary.is_some(), 
