...
impl HttpService for DefaultHttpService {
    async fn make_request(
        ...
        self.http_manager
            ...
            .await
            .map_err(|e| ServiceError::ExecutionError(JsExecutionError::ExecutionError(e.to_string())))
    }
}
    ...
    async fn run_task_tests(&self, task_path: &str) -> ServiceResult<crate::test::TestSummary> {
        crate::test::run_tests(task_path)
            ...
            .map_err(|e| match e {
                ...
                crate::test::TestError::ExecutionError(js_err) => ServiceError::ExecutionError(js_err),
                crate::test::TestError::InvalidTestFile(msg) => ServiceError::InvalidInput(msg),
                crate::test::TestError::NoTestsDirectory => ServiceError::InvalidInput("No tests directory found".to_string()),
                crate::test::TestError::IoError(e) => ServiceError::TaskError(TaskError::FileReadError(e)),
                crate::test::TestError::JsonParseError(e) => ServiceError::TaskError(TaskError::JsonParseError(e)),
            })
    }
}
...
impl RatchetEngine {
    ...
    pub async fn execute_task_from_path(
        ...
        self.services.task_service().execute_task(&mut task, input).await
    }
    
    /// Validate a task from filesystem path
    pub async fn validate_task_from_path(&self, task_path: &str) -> ServiceResult<Task> {
        let mut task = self.services.task_service().load_task(task_path).await?;
        self.services.task_service().validate_task(&mut task).await?;
        Ok(task)
    }
...
mod tests {
    ...
    impl TaskService for MockTaskService {
        async fn load_task(&self, _path: &str) -> ServiceResult<Task> {
            if self.should_fail {
                Err(ServiceError::TaskError(TaskError::TaskFileNotFound("mock error".to_string())))
            } else {
                // Return error to avoid file dependency in test
                Err(ServiceError::InvalidInput("Mock service - use real service for actual task loading".to_string()))
            }
        }
        ...
        async fn validate_task(&self, _task: &mut Task) -> ServiceResult<()> {
            if self.should_fail {
                Err(ServiceError::TaskError(TaskError::InvalidTaskStructure("mock error".to_string())))
            } else {
                Ok(())
            }
        }
        
        async fn run_task_tests(&self, _task_path: &str) -> ServiceResult<crate::test::TestSummary> {
    ...
    async fn test_error_handling() {
        ...
        // Test error handling
        let result = engine.execute_task_from_path("test", json!({"test": "data"})).await;
        assert!(result.is_err());
    }
}