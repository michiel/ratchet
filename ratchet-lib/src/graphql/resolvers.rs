...
impl Query {
    ...
    async fn tasks(
        ...
        if let Some(sync_service) = &context.task_sync_service {
            let unified_tasks = sync_service.list_all_tasks().await.map_err(|e| {
    ...
    async fn task(
        ...
        if let Some(sync_service) = &context.task_sync_service {
            // Use unified view
            let task = sync_service
                .get_task(uuid, version.as_deref())
                .await
    ...
    async fn jobs(
        ...
        let all_jobs: Vec<UnifiedJob> = db_jobs.into_iter().map(UnifiedJob::from).collect();
    ...
    async fn health(&self, ctx: &Context<'_>) -> Result<HealthStatus> {
        ...
        let message = if all_healthy {
            ...
            if !job_queue_healthy {
                issues.push("job_queue");
            }
            if !executor_healthy {
                issues.push("task_executor");
            }
...
impl Mutation {
    ...
    async fn execute_task(&self, ctx: &Context<'_>, input: ExecuteTaskInput) -> Result<UnifiedJob> {
        let context = ctx.data::<GraphQLContext>()?;
    ...
    async fn update_task_status(
        ...
        let task_id = id.as_i32().ok_or_else(|| Error::new("Invalid task ID"))?;
    ...
    async fn test_output_destinations(
        ...
        let configs: Result<Vec<OutputDestinationConfig>, _> = input
            .destinations
            .into_iter()
            .map(convert_output_destination_input)
            .collect();
        ...
        let configs =
            configs.map_err(|e| Error::new(format!("Invalid destination configuration: {}", e)))?;
        ...
        Ok(test_results
            .into_iter()
            .map(|result| TestDestinationResult {
...
fn convert_output_destination_config(config: OutputDestinationConfig) -> OutputDestination {
    match config {
        ...
        OutputDestinationConfig::S3 { .. } => OutputDestination::S3(S3Destination {
...
fn convert_output_destination_input(
    ...
    match input.destination_type {
        DestinationType::Filesystem => {
            let fs = input
                .filesystem
                .ok_or("Filesystem configuration required")?;
        ...
        DestinationType::Webhook => {
            ...
            Ok(OutputDestinationConfig::Webhook {
                ...
                timeout: std::time::Duration::from_secs(
                    webhook.timeout_seconds.unwrap_or(30) as u64
                ),
                retry_policy: crate::output::RetryPolicy {
                    max_attempts: webhook
                        .retry_policy
                        .as_ref()
                        .map(|r| r.max_attempts as u32)
                        .unwrap_or(3),
                    initial_delay: std::time::Duration::from_millis(
                        webhook
                            .retry_policy
                            .as_ref()
                            .map(|r| r.initial_delay_ms as u64)
                            .unwrap_or(1000),
                    ),
                    max_delay: std::time::Duration::from_millis(
                        webhook
                            .retry_policy
                            .as_ref()
                            .map(|r| r.max_delay_ms as u64)
                            .unwrap_or(30000),
                    ),
                    backoff_multiplier: webhook
                        .retry_policy
                        .as_ref()
        ...
        DestinationType::Database => Err("Database destinations not yet implemented".to_string()),
        DestinationType::S3 => Err("S3 destinations not yet implemented".to_string()),
    }
}
