...
impl JobQueue for JobQueueManager {
    ...
    async fn enqueue_scheduled_job(
        ...
        info!("Enqueued scheduled job {} for task {} from schedule {} to process at {}", 
    ...
    async fn cancel_job(&self, job_id: i32) -> Result<(), JobQueueError> {
        ...
        match job.status {
            ...
            _ => Err(JobQueueError::InvalidState(
                format!("Cannot cancel job {} with status {:?}", job_id, job.status)
            )),
        }
    }
    ...
    async fn retry_job(&self, job_id: i32) -> Result<(), JobQueueError> {
        ...
        match job.status {
            ...
            _ => Err(JobQueueError::InvalidState(
                format!("Cannot retry job {} with status {:?}", job_id, job.status)
            )),
        }
    }
}
...
impl JobQueue for CachedJobQueueManager {
    async fn enqueue_job(
        ...
        let result = self.inner.enqueue_job(task_id, input_data, priority).await?;
    ...
    async fn enqueue_scheduled_job(
        ...
        self.inner.enqueue_scheduled_job(task_id, schedule_id, input_data, process_at).await
    }
