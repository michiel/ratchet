...
pub trait IpcTransport {
    ...
    async fn send<T: Serialize + Send + Sync>(&mut self, message: &MessageEnvelope<T>) -> Result<(), Self::Error>;
    
    /// Receive a message from the other end
    async fn receive<T: for<'de> Deserialize<'de>>(&mut self) -> Result<MessageEnvelope<T>, Self::Error>;
...
mod tests {
    ...
    fn test_coordinator_message_serialization() {
        ...
        let envelope = MessageEnvelope::new(message);
        let json = serde_json::to_string(&envelope).unwrap();
        let _deserialized: MessageEnvelope<CoordinatorMessage> = serde_json::from_str(&json).unwrap();
    }
}