...
impl ProcessTaskExecutor {
    ...
    pub async fn execute_task_send(
        ...
        match result {
            Ok(task_result) => {
                let output_for_db = task_result
                    .output
                    .clone()
                    .unwrap_or_else(|| serde_json::json!({}));
                execution_repo
                    .mark_completed(execution_id, output_for_db)
                    .await?;
            ...
            Err(e) => {
                execution_repo
                    .mark_failed(execution_id, e.to_string(), None)
                    .await?;
        ...
        match result {
            Ok(task_result) => {
                let output_for_db = task_result
                    ...
                    .unwrap_or_else(|| serde_json::json!({}));
                execution_repo
                    .mark_completed(execution_id, output_for_db.clone())
                    .await?;
    ...
    pub async fn health_check_send(&self) -> Result<(), ExecutionError> {
        ...
        for result in health_results {
            if let Err(e) = result {
                return Err(ExecutionError::TaskExecutionError(format!(
    ...
    async fn deliver_output(
        ...
        if let Some(destinations_json) = &job_entity.output_destinations {
            ...
            let destinations: Vec<OutputDestinationConfig> =
                serde_json::from_value(destinations_json.clone()).map_err(|e| {
                    ExecutionError::ExecutionFailed(format!(
                        "Failed to parse output destinations: {}",
                        e
                    ))
                })?;
            ...
            if !destinations.is_empty() {
                ...
                let delivery_manager = OutputDeliveryManager::from_configs(&destinations, 10)
                    .map_err(|e| {
                        ExecutionError::ExecutionFailed(format!(
                            "Failed to create delivery manager: {}",
                            e
                        ))
                    })?;
...
impl ProcessTaskExecutor {
    ...
    async fn execute_task_in_worker(
        ...
        use crate::execution::ipc::{CoordinatorMessage, ExecutionContext, WorkerMessage};
        use tokio::time::Duration;
        use uuid::Uuid;

        let correlation_id = Uuid::new_v4();
        let execution_context =
            ExecutionContext::new(execution_uuid, job_uuid, task_uuid, task_version);
        ...
        match result {
            ...
            Ok(CoordinatorMessage::Error { error, .. }) => Err(ExecutionError::TaskExecutionError(
                format!("Worker error: {:?}", error),
            )),
            Ok(_) => Err(ExecutionError::TaskExecutionError(
                "Unexpected response from worker".to_string(),
            )),
            Err(e) => Err(ExecutionError::TaskExecutionError(format!(
                "IPC error: {}",
                e
            ))),
        }
...
mod tests {
    ...
    async fn create_test_task_in_db(repositories: &RepositoryFactory) -> (i32, String) {
        ...
        fs::write(task_path.join("main.js"), main_js).expect("Failed to write main.js");
    ...
    async fn test_process_executor_start_stop() {
        ...
        let executor = ProcessTaskExecutor::new(repositories, config)
            .await
            .unwrap();
    ...
    async fn test_execute_task_send_with_database_integration() {
        ...
        let executor = ProcessTaskExecutor::new(repositories.clone(), config)
            .await
            .unwrap();
        ...
        let result = executor
            .execute_task_send(task_id, input_data.clone(), None)
            .await;
    ...
    async fn test_worker_manager_integration() {
        ...
        let _executor = ProcessTaskExecutor::new(repositories, config)
            .await
            .unwrap();
        ...
        assert!(
            start_result.is_ok(),
            "Worker start should succeed or fail gracefully"
        );
    ...
    async fn test_ipc_message_handling() {
        ...
        let _executor = ProcessTaskExecutor::new(repositories.clone(), config)
            .await
            .unwrap();
        ...
        if let Ok(WorkerMessage::ExecuteTask {
            task_id: tid,
            input_data: input,
            ..
        }) = deserialized
    ...
    async fn test_concurrent_task_execution() {
        ...
        let handle1 = tokio::spawn(async move {
            executor1
                .execute_task_send(task_id1, json!({ "num1": 1, "num2": 2 }), None)
                .await
        });

        let handle2 = tokio::spawn(async move {
            executor2
                .execute_task_send(task_id2, json!({ "num1": 3, "num2": 4 }), None)
                .await
        });
        ...
        assert_ne!(
            exec_result1.unwrap().execution_id,
            exec_result2.unwrap().execution_id
        );
    }
    ...
    async fn test_task_executor_trait_implementation() {
        ...
        let result = executor
            .execute_task(task_id, input_data.clone(), None)
            .await;
        assert!(result.is_ok());
