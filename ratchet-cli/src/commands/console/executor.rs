...
impl CommandExecutor {
    ...
    async fn execute_task_command(&self, command: ConsoleCommand) -> Result<CommandResult> {
        match command.action.as_str() {
            ...
            _ => Ok(CommandResult::Error {
                message: format!(
                    "Unknown task command: '{}'. Available commands: list, show, enable, disable, execute",
                    command.action
                ),
            }),
        }
    ...
    async fn execute_server_command(&self, command: ConsoleCommand) -> Result<CommandResult> {
        match command.action.as_str() {
            ...
            "workers" => Ok(CommandResult::Table {
                headers: vec![
                    "ID".to_string(),
                    "Status".to_string(),
                    "Tasks".to_string(),
            ...
            _ => Ok(CommandResult::Error {
                message: format!(
                    "Unknown server command: '{}'. Available commands: status, workers, metrics",
                    command.action
                ),
            }),
        }
    ...
    async fn execute_monitor_command(&self, command: ConsoleCommand) -> Result<CommandResult> {
        ...
        if !command.action.is_empty() && command.action != "start" {
            return Ok(CommandResult::Error {
                message: format!(
                    "Unknown monitor command: {}. Try 'monitor' or 'monitor start'",
                    command.action
                ),
            });
        }
