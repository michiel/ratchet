...
use crate::types::{DiscoveredTask, SyncResult, TaskReference, SyncError};
...
impl DatabaseSync {
    ...
    pub async fn sync_discovered_tasks(&self, tasks: Vec<DiscoveredTask>) -> Result<SyncResult> {
        ...
        for discovered_task in tasks {
            match self.sync_single_task(&discovered_task).await {
                ...
                Err(e) => {
                    error!("Failed to sync task {} v{}: {}", 
                        discovered_task.metadata.name, 
                        discovered_task.metadata.version, 
                        e);
        ...
        info!("Database sync completed: {} added, {} updated, {} errors", 
    ...
    async fn sync_single_task(&self, discovered_task: &DiscoveredTask) -> Result<SyncType> {
        ...
        let existing_task = all_tasks.into_iter()
            .find(|task| task.name == discovered_task.metadata.name && 
                         task.version == discovered_task.metadata.version);
        ...
        match existing_task {
            Some(existing) => {
                ...
                match resolution {
                    ConflictResolution::UseRegistry => {
                        ...
                        info!("Updated task {} v{}", discovered_task.metadata.name, discovered_task.metadata.version);
                        Ok(SyncType::Updated)
                    }
                    ConflictResolution::UseDatabase => {
                        info!("Skipped task {} v{} (database version preferred)", 
                    ...
                    ConflictResolution::Merge => {
                        ...
                        info!("Merged task {} v{}", discovered_task.metadata.name, discovered_task.metadata.version);
                        Ok(SyncType::Updated)
                    }
                }
            }
            None => {
                ...
                task_repo.create(new_task).await
                    .map_err(|e| RegistryError::Other(e.to_string()))?;
                
                info!("Added task {} v{}", discovered_task.metadata.name, discovered_task.metadata.version);
                Ok(SyncType::Added)
            }
        }
    }
    
    fn convert_discovered_to_task_model(&self, discovered: &DiscoveredTask, existing_id: Option<i32>) -> Result<tasks::Model> {
        let now = chrono::Utc::now();
        
        // Generate a path from the task reference and source
        let path = format!("{}/{}/{}", discovered.task_ref.source, discovered.task_ref.name, discovered.task_ref.version);
        
        Ok(tasks::Model {
            id: existing_id.unwrap_or(0), // Will be auto-generated for new tasks
            uuid: existing_id.map(|_| discovered.metadata.uuid).unwrap_or(discovered.metadata.uuid),
            ...
            path,
            metadata: serde_json::to_value(&discovered.metadata)
            ...
            created_at: existing_id.map(|_| discovered.metadata.created_at).unwrap_or(discovered.metadata.created_at),
