...
use crate::{
    error::RegistryError,
    config::TaskSource,
    types::{DiscoveredTask, TaskDefinition, TaskReference, TaskMetadata},
};
...
impl EmbeddedLoader {
    ...
    fn convert_to_task_definition(&self, embedded_task: &EmbeddedTask, reference: TaskReference) -> crate::error::Result<TaskDefinition> {
        // Parse metadata
        ...
        let uuid_str = metadata_json.get("uuid")
            .and_then(|v| v.as_str())
        ...
        let tags = metadata_json.get("tags")
...
impl crate::loaders::TaskLoader for EmbeddedLoader {
    async fn discover_tasks(&self, _source: &TaskSource) -> crate::error::Result<Vec<DiscoveredTask>> {
        ...
        for embedded_task in self.tasks.values() {
            ...
            let version = metadata_json.get("version")
                .and_then(|v| v.as_str())
    ...
    async fn load_task(&self, task_ref: &TaskReference) -> crate::error::Result<TaskDefinition> {
        let embedded_task = self.tasks.get(&task_ref.name)
...
mod tests {
    use super::*;
    use crate::config::{TaskSource, GitConfig};
