...
async fn test_mcp_api_workflow() -> Result<()> {
    ...
    let tool_names: std::collections::HashSet<&str> = tools.iter()
    ...
    for expected_tool in &expected_tools {
        assert!(tool_names.contains(expected_tool), "Missing expected tool: {}", expected_tool);
    }
    ...
    if list_result["isError"].as_bool().unwrap_or(false) {
        println!("⚠️ List tasks returned error (expected in test environment): {}", 
    ...
    assert!(execute_result["isError"].as_bool().unwrap_or(false), 
    ...
    if status_result["isError"].as_bool().unwrap_or(false) {
        ...
        println!("ℹ️ Execution status returned result: {}", status_result["content"][0]["text"]);
    }
    ...
    if batch_result["isError"].as_bool().unwrap_or(false) {
        ...
        println!("ℹ️ Batch execution handled empty requests: {}", batch_result["content"][0]["text"]);
    }
...
async fn test_mcp_parameter_validation() -> Result<()> {
    ...
    println!("ℹ️ Parameter type validation result: {}", 
            if result["isError"].as_bool().unwrap_or(false) { "strict" } else { "lenient" });
