...
pub fn extract_task_filters(filters: &HashMap<String, String>) -> TaskFilters {
    TaskFilters {
        ...
        updated_before: filters
            .get("updated_before")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        validated_before: filters
            .get("validated_before")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
...
pub fn extract_execution_filters(filters: &HashMap<String, String>) -> ExecutionFilters {
    ExecutionFilters {
        ...
        queued_after: filters
            .get("queued_after")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        completed_after: filters
            .get("completed_after")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        ...
        task_id_in: filters
            .get("task_id_in")
            .map(|v| v.split(',').filter_map(|s| parse_api_id(s.trim())).collect()),
        id_in: filters
            .get("id_in")
            .map(|v| v.split(',').filter_map(|s| parse_api_id(s.trim())).collect()),
        ...
        status_in: filters
            .get("status_in")
            .map(|v| v.split(',').filter_map(|s| parse_execution_status(s.trim())).collect()),
        status_not: filters.get("status_ne").and_then(|v| parse_execution_status(v)),

        // Extended date filtering
        queued_before: filters
            .get("queued_before")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        started_after: filters
            .get("started_after")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        started_before: filters
            .get("started_before")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        completed_before: filters
...
pub fn extract_job_filters(filters: &HashMap<String, String>) -> JobFilters {
    JobFilters {
        ...
        queued_after: filters
            .get("queued_after")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        scheduled_before: filters
            .get("scheduled_before")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        ...
        status_in: filters
            .get("status_in")
            .map(|v| v.split(',').filter_map(|s| parse_job_status(s.trim())).collect()),
        status_not: filters.get("status_ne").and_then(|v| parse_job_status(v)),

        // Advanced priority filtering
        priority_in: filters
            .get("priority_in")
            .map(|v| v.split(',').filter_map(|s| parse_job_priority(s.trim())).collect()),
...
pub fn extract_schedule_filters(filters: &HashMap<String, String>) -> ScheduleFilters {
    ScheduleFilters {
        ...
        next_run_after: filters
            .get("next_run_after")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        last_run_after: filters
            .get("last_run_after")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
        last_run_before: filters
            .get("last_run_before")
            .and_then(|v| DateTime::parse_from_rfc3339(v).ok().map(|dt| dt.with_timezone(&Utc))),
