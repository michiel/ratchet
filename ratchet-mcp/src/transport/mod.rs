...
pub use connection::{ConnectionPool, ConnectionHealth, HealthMonitor};
...
impl TransportFactory {
    ...
    pub async fn create(config: TransportType) -> McpResult<Box<dyn McpTransport>> {
        match config {
            TransportType::Stdio { command, args, env, cwd } => {
                Ok(Box::new(StdioTransport::new(command, args, env, cwd)?))
            }
            TransportType::Sse { url, headers, auth, timeout, verify_ssl } => {
