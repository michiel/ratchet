...
use crate::{
    ...
    protocol::{JsonRpcRequest, InitializeParams, ClientCapabilities, ClientInfo},
    transport::{SseTransport, McpTransport, TransportType, SseAuth},
    security::McpAuth,
};
...
fn test_sse_transport_configuration() {
    let config = TransportType::Sse {
        ...
        auth: Some(SseAuth::Bearer { token: "test-token".to_string() }),
