...
impl ApiError {
    ...
    pub fn unauthorized(reason: Option<&str>) -> Self {
        ...
        Self::new("UNAUTHORIZED", message).with_suggestions(vec![
    ...
    pub fn forbidden(reason: Option<&str>) -> Self {
        ...
        Self::new("FORBIDDEN", message).with_suggestions(vec![
    ...
    pub fn rate_limited(retry_after: Option<u64>) -> Self {
        ...
        Self::new("RATE_LIMITED", message).with_suggestions(vec![
    ...
    pub fn service_unavailable(reason: Option<&str>) -> Self {
        ...
        Self::new("SERVICE_UNAVAILABLE", message).with_suggestions(vec![
    ...
    pub fn timeout(operation: &str) -> Self {
        Self::new("TIMEOUT", format!("{} operation timed out", operation)).with_suggestions(vec![
...
// GraphQL Error conversion (only when graphql feature is enabled)
#[cfg(feature = "graphql")]
...
/// Common error types that can be converted to ApiError
#[derive(Debug, Error)]
...
impl From<CommonError> for ApiError {
    fn from(error: CommonError) -> Self {
        match error {
            CommonError::Validation(msg) => ApiError::validation_error("unknown", &msg),
            CommonError::NotFound(msg) => ApiError::not_found("resource", &msg),
            CommonError::Internal(msg) => ApiError::internal_error(msg),
            CommonError::Database(msg) => ApiError::internal_error(format!("Database error: {}", msg)),
            CommonError::Network(msg) => ApiError::service_unavailable(Some(&msg)),
        }
    }
}
