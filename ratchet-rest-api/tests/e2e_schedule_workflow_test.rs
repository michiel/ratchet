...
async fn test_complete_schedule_workflow() -> Result<()> {
    ...
    let jobs = jobs_data["data"].as_array()
    ...
    for (i, job) in jobs.iter().enumerate() {
        ...
        assert_eq!(job["scheduleId"], schedule_id, "Job {} should reference our schedule", i);
        
        println!("✅ Job {} validated: status={}, priority={}", 
    ...
    for job in jobs {
        ...
        if executions_response.status() == 200 {
            ...
            let executions = executions_data["data"].as_array()
    ...
    println!("✅ Found {} execution(s), {} successful", execution_count, successful_executions);
    ...
    let all_schedules = all_schedules_data["data"].as_array()
        .expect("Schedules data should be an array");
    
    let found_deleted_schedule = all_schedules.iter()
    ...
    println!("  ⚡ Executions: {} total, {} successful", execution_count, successful_executions);
...
async fn test_schedule_crud_operations() -> Result<()> {
    ...
    assert!(delete_response.status().is_success(), "Schedule deletion should succeed");
    ...
    assert_eq!(verify_delete_response.status(), 404, "Schedule should not exist after deletion");
