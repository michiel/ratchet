...
fn validate_refine_response(response: &Value) -> Result<()> {
    ...
    assert!(
        pagination.get("totalPages").is_some(),
        "Pagination should have 'totalPages'"
    );
    assert!(pagination.get("hasNext").is_some(), "Pagination should have 'hasNext'");
    assert!(
        pagination.get("hasPrevious").is_some(),
        "Pagination should have 'hasPrevious'"
    );
...
async fn test_refine_pagination_start_end() -> Result<()> {
    ...
    let response = client.get(format!("{}/tasks?_start=0&_end=5", API_BASE)).send().await?;
...
async fn test_refine_sorting() -> Result<()> {
    ...
    let response = client
        .get(format!(
            "{}/tasks?_start=0&_end=10&_sort=created_at&_order=DESC",
            API_BASE
        ))
        .send()
        .await?;
...
async fn test_refine_advanced_filtering() -> Result<()> {
    ...
    for (filter_key, filter_value) in filter_tests {
        let response = client
            .get(format!(
                "{}/tasks?_start=0&_end=10&{}={}",
                API_BASE, filter_key, filter_value
            ))
            .send()
            .await?;
...
async fn test_all_endpoints_refine_compatibility() -> Result<()> {
    ...
    let endpoints = vec!["tasks", "executions", "jobs", "schedules"];
    ...
    for endpoint in endpoints {
        ...
        assert!(
            response.status().is_success(),
            "Endpoint {} should support Refine.dev pagination",
            endpoint
        );
        ...
        assert!(
            response.status().is_success(),
            "Endpoint {} should support Refine.dev sorting",
            endpoint
        );
...
async fn test_pagination_metadata_accuracy() -> Result<()> {
    ...
    // Get first page
    let response = client.get(format!("{}/tasks?_start=0&_end=3", API_BASE)).send().await?;
...
async fn test_response_format_compliance() -> Result<()> {
    ...
    for field in required_pagination_fields {
        assert!(
            pagination_obj.contains_key(field),
            "Pagination must have '{}' field",
            field
        );
    }
    ...
    assert!(
        pagination.get("totalPages").unwrap().is_number(),
        "totalPages should be number"
    );
    assert!(
        pagination.get("hasNext").unwrap().is_boolean(),
        "hasNext should be boolean"
    );
    assert!(
        pagination.get("hasPrevious").unwrap().is_boolean(),
        "hasPrevious should be boolean"
    );
