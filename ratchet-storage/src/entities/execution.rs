...
impl Execution {
    ...
    pub fn add_http_request(&mut self, request_info: serde_json::Value) {
        let mut requests = self.http_requests.take().unwrap_or_else(|| serde_json::json!([]));
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(crate::StorageError::ValidationFailed(
                format!("Invalid execution status: {}", s)
            )),
        }
    }
}
...
mod tests {
    ...
    fn test_execution_timeout() {
        ...
        assert!(execution.is_failed());
        assert!(execution.is_terminal());
        assert_eq!(execution.error_message, Some("Execution timed out".to_string()));
    }
