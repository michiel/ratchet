...
impl SafeFilterBuilder {
    ...
    pub fn in_values(mut self, field: impl Into<String>, values: Vec<FilterValue>) -> StorageResult<Self> {
        if values.is_empty() {
            return Err(StorageError::ValidationFailed("IN filter cannot have empty values".to_string()));
        }
        if values.len() > 100 {
            return Err(StorageError::ValidationFailed("IN filter cannot have more than 100 values".to_string()));
        }
    ...
    fn add_filter(&mut self, field: String, operator: FilterOperator, value: FilterValue) -> StorageResult<()> {
        ...
        if self.filters.len() >= self.max_conditions {
            return Err(StorageError::ValidationFailed(
                format!("Too many filter conditions (max: {})", self.max_conditions)
            ));
        }
        ...
        if !self.allowed_fields.is_empty() && !self.allowed_fields.contains(&field) {
            return Err(StorageError::ValidationFailed(
                format!("Field '{}' is not allowed for filtering", field)
            ));
        }
    ...
    fn validate_field_name(&self, field: &str) -> StorageResult<()> {
        ...
        let dangerous_patterns = [
            ...
            "exec", "execute", "sp_", "xp_", "alter", "create"
        ];
        
        for pattern in &dangerous_patterns {
            if lower_field.contains(pattern) {
                return Err(StorageError::ValidationFailed(
                    format!("Field name contains potentially dangerous pattern: {}", pattern)
                ));
            }
        }
            ...
            FilterValue::String(s) => {
                ...
                for pattern in &dangerous_patterns {
                    if s.contains(pattern) {
                        return Err(StorageError::ValidationFailed(
                            format!("String contains potentially dangerous pattern: {}", pattern)
                        ));
                    }
                }
            },
...
mod tests {
    ...
    fn test_max_conditions_limit() {
        ...
        builder = builder.equals("field1", FilterValue::String("value1".to_string())).unwrap();
        builder = builder.equals("field2", FilterValue::String("value2".to_string())).unwrap();
    ...
    fn test_allowed_fields() {
        let builder = SafeFilterBuilder::new()
            .with_allowed_fields(vec!["name".to_string()]);
