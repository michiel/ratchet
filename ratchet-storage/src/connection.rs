...
use crate::{StorageError, StorageResult, StorageConfig};
...
pub trait Connection: Send + Sync {
    ...
    async fn fetch_json(&self, query: &str, params: &[serde_json::Value]) -> StorageResult<Vec<serde_json::Value>>;
    
    /// Execute a query that returns a single JSON row
    async fn fetch_one_json(&self, query: &str, params: &[serde_json::Value]) -> StorageResult<serde_json::Value>;
    
    /// Execute a query that may return zero or one JSON row
    async fn fetch_optional_json(&self, query: &str, params: &[serde_json::Value]) -> StorageResult<Option<serde_json::Value>>;
...
pub trait Transaction: Send + Sync {
    ...
    async fn fetch_json(&mut self, query: &str, params: &[serde_json::Value]) -> StorageResult<Vec<serde_json::Value>>;
    
    /// Fetch one JSON row within the transaction
    async fn fetch_one_json(&mut self, query: &str, params: &[serde_json::Value]) -> StorageResult<serde_json::Value>;
    
    /// Fetch optional JSON row within the transaction
    async fn fetch_optional_json(&mut self, query: &str, params: &[serde_json::Value]) -> StorageResult<Option<serde_json::Value>>;
...
impl Connection for InMemoryConnection {
    ...
    async fn fetch_json(&self, _query: &str, _params: &[serde_json::Value]) -> StorageResult<Vec<serde_json::Value>> {
        self.increment_query_count().await;
        Ok(Vec::new())
    }
    
    async fn fetch_one_json(&self, _query: &str, _params: &[serde_json::Value]) -> StorageResult<serde_json::Value> {
        self.increment_query_count().await;
        Err(StorageError::NotFound)
    }
    
    async fn fetch_optional_json(&self, _query: &str, _params: &[serde_json::Value]) -> StorageResult<Option<serde_json::Value>> {
        self.increment_query_count().await;
        Ok(None)
    }
...
pub async fn create_connection_manager(config: &StorageConfig) -> StorageResult<Arc<dyn ConnectionManager>> {
    config.validate()?;
    
    match &config.backend {
        crate::config::StorageBackend::InMemory => {
            Ok(Arc::new(InMemoryConnectionManager::new()))
        }
