...
impl BaseRepository<Schedule> for ScheduleRepository {
    async fn create(&self, entity: &Schedule) -> StorageResult<Schedule> {
        Ok(entity.clone())
    }
    async fn find_by_id(&self, _id: i32) -> StorageResult<Option<Schedule>> {
        Ok(None)
    }
    async fn find_by_uuid(&self, _uuid: Uuid) -> StorageResult<Option<Schedule>> {
        Ok(None)
    }
    async fn update(&self, entity: &Schedule) -> StorageResult<Schedule> {
        Ok(entity.clone())
    }
    ...
    async fn find_all(&self, _query: &Query) -> StorageResult<Vec<Schedule>> {
        Ok(Vec::new())
    }
    ...
    async fn batch_create(&self, entities: &[Schedule]) -> StorageResult<Vec<Schedule>> {
        Ok(entities.to_vec())
    }
    async fn batch_update(&self, entities: &[Schedule]) -> StorageResult<Vec<Schedule>> {
        Ok(entities.to_vec())
    }
