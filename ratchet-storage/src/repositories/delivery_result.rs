...
impl DeliveryResultRepository {
    ...
    pub async fn find_by_execution_id(
        &self,
        _execution_id: i32,
    ) -> StorageResult<Vec<DeliveryResult>> {
        Ok(Vec::new())
    }
...
impl BaseRepository<DeliveryResult> for DeliveryResultRepository {
    async fn create(&self, entity: &DeliveryResult) -> StorageResult<DeliveryResult> {
        Ok(entity.clone())
    }
    async fn find_by_id(&self, _id: i32) -> StorageResult<Option<DeliveryResult>> {
        Ok(None)
    }
    async fn find_by_uuid(&self, _uuid: Uuid) -> StorageResult<Option<DeliveryResult>> {
        Ok(None)
    }
    async fn update(&self, entity: &DeliveryResult) -> StorageResult<DeliveryResult> {
        Ok(entity.clone())
    }
    ...
    async fn find_all(&self, _query: &Query) -> StorageResult<Vec<DeliveryResult>> {
        Ok(Vec::new())
    }
